        ...
        <div className='flex flex-col h-full w-full'>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6 p-4">
                ...
                {katalog.map((item) => {
                    ...
                    return (
                        <div key={item.id} className="flex flex-col bg-black rounded-xl shadow-md overflow-hidden w-full">
                            <Link to={`/katalog/${item.id}`}>
                                <img className="w-full h-[200px] object-cover" src={item.slika[0]} alt={item.naziv} />
                            </Link>

                            <div className="p-4 flex flex-col justify-between flex-grow">
                                <div className="space-y-2">
                                    <p className="text-pink-500 text-xl">{item.naziv}</p>
                                    <p className="text-white text-sm">{item.opis}</p>
